...
function generateHTML(data, index) {
  ...
  <div class="div1 container ">
            ...
            <article class="mw6 center bg-white br3 pa3 pa0-ns mv3 ba b--black-20 shadow-5">
                ...
                <div class="tc mt3">
                <img src="${data.stateFlagURL}" alt= "Flag_of_${
    data.stateName
  }" class="br-100 h3 w3 dib" title="Photo of a kitty staring at you">
                <h1 class="f4">${data.stateName} - ${data.bill_id}</h1>
                <span>${
                  data.isBillNew
                    ? '<a class="f6 grow no-underline br-pill ph2 pv1 mb2 dib navy bg-light-yellow">New</a>'
                    : ""
                } 
                ${
                  data.isLastUpdateImportant
                    ? '<a class="f6 grow no-underline br-pill ph2 pv1 mb2 dib navy bg-washed-green">Major Update</a>'
                    : ""
                }
                </span>
                ...
                <div>
                    <article class="w-100 pa0">
                                    ...
                                    <h3 class="tc f4 f4-m measure-narrow lh-title mv0">
                                        ...
                                        <div class="pt1 pl3 pr3 w-100 dt dt--fixed">
                                            ...
                                            <div class="dtc h1 black  br1 br--left tc" style="width: 50%">
                                                <small class= "f7 gray">${(data.actions[0].date =
                                                  data.actions.length > 0
                                                    ? data.actions[0].date.substring(
                                                        0,
                                                        10
                                                      )
                        ...
                        <span class="f6 db pv1 pr3 truncate">${lowercaseTitle[0].toUpperCase() +
                          lowercaseTitle.substring(1)}</span>
                        </div>

                        <div class="pt3 pl3 pb0">
                        <small class="gray lh-title">Latest Action Date</small>
                        <span class="f6 db pv1"><time>${lastBillAction.date.substring(
                          0,
                          10
                        )}</time></span>
                        </div>
                        ...
                        ${data.sources
                          .map((url, i) => {
                            return `<span class="f6 db pv2"> <a href="${
                              data.sources[i].url
                            }" target="_blank" rel="noopener" class="f6 link dark-blue hover-dark-gray">${
                              data.stateName
                            } State Legislature (Link ${i + 1})</a></span>`;
                          })
                          .join("")}
  ...
  try {
    ...
    // const res = fetch("http://localhost:3000/track", {
      ...
      .then(json => {
        ...
        newBillCount.innerHTML = `New Bills (${
          Object.keys(newBillsAdded).length
        })`;
